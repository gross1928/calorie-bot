-- Создаем таблицу 'profiles', только если она не существует
...
-- Создаем таблицу 'meals', только если она не существует
...
-- Добавляем колонку 'weight_g' в таблицу 'meals', если ее нет
ALTER TABLE public.meals ADD COLUMN IF NOT EXISTS weight_g INT;

-- Добавляем колонку 'ingredients' в таблицу 'meals', если ее нет
ALTER TABLE public.meals ADD COLUMN IF NOT EXISTS ingredients TEXT[];

-- Добавляем колонки для дневной нормы КБЖУ в таблицу 'profiles', если их нет
...
-- Таблица для данных планов тренировок
CREATE TABLE IF NOT EXISTS public.workout_plan_data (
    ...
    priority_zones TEXT[], -- массив зон: back, chest, legs, shoulders, core
    injuries TEXT, -- описание травм или 'none'
    location TEXT NOT NULL, -- home, gym, outdoor
    frequency_per_week INT NOT NULL, -- количество тренировок в неделю
    duration_minutes INT NOT NULL, -- продолжительность одной тренировки
    preferred_types TEXT[], -- массив типов: strength, cardio, hiit, yoga, stretching
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- Таблица для данных планов питания
...
-- Создаём таблицу water_intake с правильными типами данных
...
-- Политики RLS
CREATE POLICY "Пользователи видят свои записи" ON water_intake
    FOR ALL USING (true);
...
-- 1. Таблица еженедельных челленджей
CREATE TABLE IF NOT EXISTS weekly_challenges (
    id BIGSERIAL PRIMARY KEY,
    week_start TIMESTAMPTZ NOT NULL UNIQUE, -- Начало недели (понедельник)
    title TEXT NOT NULL,                    -- Название челленджа
    description TEXT NOT NULL,              -- Описание челленджа
    target_value INTEGER NOT NULL,          -- Целевое значение (например, 70000 шагов)
    unit TEXT NOT NULL,                     -- Единица измерения (шагов, литров, минут, км, раз)
    type TEXT NOT NULL,                     -- Тип челленджа (steps, water, workout_time, distance, exercises)
    motivation TEXT NOT NULL,               -- Мотивационное сообщение
...
-- 2. Таблица отслеживания шагов
CREATE TABLE IF NOT EXISTS steps_tracking (
    id BIGSERIAL PRIMARY KEY,
    user_id BIGINT NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
    date DATE NOT NULL,                     -- Дата (только дата, без времени)
    steps INTEGER NOT NULL DEFAULT 0,       -- Количество шагов за день
    updated_at TIMESTAMPTZ DEFAULT NOW(),   -- Последнее обновление
    
    -- Уникальность: один пользователь может иметь только одну запись на дату
    UNIQUE(user_id, date)
);
...
-- 3. Добавляем поле для уведомлений в таблицу profiles (если еще нет)
ALTER TABLE profiles 
ADD COLUMN IF NOT EXISTS notifications_enabled BOOLEAN DEFAULT true;

-- Создаем индекс для поиска пользователей с включенными уведомлениями
...
-- Создаем триггер для автоматического обновления updated_at в steps_tracking
...
-- 5. Включаем Row Level Security (RLS) - упрощенная версия
ALTER TABLE weekly_challenges ENABLE ROW LEVEL SECURITY;
ALTER TABLE steps_tracking ENABLE ROW LEVEL SECURITY;

-- 6. Простые политики доступа для weekly_challenges
...
-- 7. Простые политики доступа для steps_tracking
...
COMMENT ON TABLE weekly_challenges IS 'Еженедельные челленджи для пользователей';
COMMENT ON TABLE steps_tracking IS 'Отслеживание шагов пользователей по дням';

COMMENT ON COLUMN weekly_challenges.week_start IS 'Дата начала недели (понедельник) для челленджа';
COMMENT ON COLUMN weekly_challenges.target_value IS 'Целевое значение для выполнения челленджа';
COMMENT ON COLUMN weekly_challenges.type IS 'Тип челленджа: steps, water, workout_time, distance, exercises';

COMMENT ON COLUMN steps_tracking.date IS 'Дата учета шагов (только дата, без времени)';
COMMENT ON COLUMN steps_tracking.steps IS 'Количество шагов за указанную дату';

-- 9. Создаем тестовый челлендж на текущую неделю
...
VALUES 
    (
        ...
        'Активность - основа здоровья! Двигайтесь каждый день и достигните 70,000 шагов за неделю. Это примерно 10,000 шагов в день.',
        70000,
        'шагов',
        'steps',
        'Каждый шаг приближает вас к цели! Вы сможете это сделать! 💪'
    )
ON CONFLICT (week_start) DO NOTHING;

-- Успешное создание всех таблиц
SELECT 'Все таблицы для системы челленджей успешно созданы! 🎉' as result; 
